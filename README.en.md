...
- `healthCheck` sends protocol-correct probes every `intervalMs` (Responses/Chat/Messages use their own path, request body, and authentication headers); without an explicit `healthCheck.models` list it scans exact models and expands `modelPatterns` entries with string or array endpoints. Scheduled cycles are single-flight, non-stream bodies are released, and hot reload/stop aborts probes that are still in flight. Failed providers move to the end of the candidate order for new sessions (`provider_health` events). Different credentials on one URL are probed independently.
