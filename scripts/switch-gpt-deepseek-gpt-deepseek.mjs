...
async function main() {
  ...
  try {
    let r = await request(target, MODELS.gpt, history); assertNoInvalid('hop1 gpt', r);
    history.push(reasoningItem('rs1', 'thinking'));
    history.push(assistantItem('a1', 'ok'));
    r = await request(target, MODELS.deepseek, history, true); assertNoInvalid('hop2 deepseek', r);
    history.push(toolCallItem('fc1', 'call_1', TOOL_NAME));
    history.push(toolOutputItem('fco1', 'call_1', 'ok'));
    r = await request(target, MODELS.gpt, history); assertNoInvalid('hop3 gpt', r);
    history.push(reasoningItem('rs2', 'more thinking'));
    history.push(assistantItem('a2', 'done'));
    r = await request(target, MODELS.deepseek, history, true); assertNoInvalid('hop4 deepseek', r);
    console.log((useLive ? 'live' : isMock ? 'mock' : 'live-default') + ' OK: GPT->DeepSeek->GPT->DeepSeek no invalid');
  } finally {
